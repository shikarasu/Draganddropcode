...
class NewProjectList extends Component {
  ...
  onDragEnd = result => {
    ...
    const newProject = Object.assign([], this.state.newProject);
    const project = this.state.newProject[source.index];
    newProject.splice(source.index, 1);
    newProject.splice(destination.index, 0, project);
    this.setState({
      newProject
    });
  }
  ...
  render(){
    ...
    const { NewProject, selectedRowIds, reorderEnabled } = this.state;
            ...
            {(provided) => (
              <Ref innerRef={provided.innerRef}>
                <Table.Body {...provided.droppableProps}>
                  {this.state.newProject.map((project, idx) => {
                    return (
                      <Draggable
